...
<h2 class="render-title <?php echo (osc_get_preference('footer_link', 'summersday') ? '' : 'separate-top'); ?>"><?php _e('Map Options', 'summersday'); ?></h2>
<div class="flashmessage flashmessage-warning flashmessage-inline"><p><strong><?php _e('How to assign a region to your map?','summersday'); ?></strong><br><?php _e('First, click in the map to select the area you want to set. Then, a dialog will appear and you only need to select the region you want to assign. Finally, once you have selected the region, click the save button.','summersday'); ?></p></div>
...
function countrySelect($id = null,$selected = null){
    ...
    $echo .= '<option value="">' . __('Select a region', 'summersday') . '</option>';
...
for($i=0;$i<0;$i++){
    ...
    <form id="region-<?php echo $i; ?>" class="region-dialog has-form-actions" onsubmit="return validateForm('region-<?php echo $i; ?>')" >
        <div class="form-horizontal">
            <div class="form-row">
                <div class="form-label"><?php _e('Select a region', 'summersday'); ?></div>
            ...
            <div style="display:none" class="form-row error">
                <div>*<?php _e('Select a region first', 'summersday'); ?></div>
            </div>
            <div class="form-actions">
                <div class="wrapper">
                <input type="submit" value="<?php _e('Save changes', 'summersday'); ?>" class="btn btn-submit">
...
$('.region-dialog').dialog({
    ...
    modal:true,
    width:500,
    title: '<?php echo osc_esc_js(__('Select a region', 'summersday')); ?>'
});
